...
>
> Draft of review feedback for the Initial LOD proposal currently under review upstream (Pixar OpenUSD-proposals, PR 81). Still working through the proposal end-to-end and gathering input from other reviewers, so this is not final feedback. Comments on the draft itself are welcome here.
>
> Note: this Issue intentionally avoids upstream PR URLs and cross-repo notation in its body, so the upstream PR timeline is not pinged. The deliberate cross-reference will come later, when the corresponding PR comment is posted upstream.
