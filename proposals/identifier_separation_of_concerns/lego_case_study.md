...
The LEGO case study makes concrete a key point from the proposal's comparison table: *"Different domains need different identifier fields. A single multi-apply schema must either carry only the common subset or accumulate rarely-used properties — the more heterogeneous the contents, the more this tension favors dictionaries."* The LEGO identifier package is substantially more heterogeneous than, say, an IFC GlobalId (a single string), and this heterogeneity shows up directly in how much of the package fits within a minimal Approach B base schema.

At the same time, the BOM traversal use case highlights where Approach B's discoverability advantage is most tangible: a tool that knows to look for `legoId:elementId` as a typed schema property can use schema-driven queries, whereas the Approach A equivalent requires knowing the key path `assetInfo["sourceIdentifiers"]["lego"]["elementId"]` in advance — which is not machine-discoverable without prior knowledge.

This suggests a possible hybrid: a well-governed `assetInfo` sub-dictionary convention for heterogeneous packages (accommodating LEGO's Design ID + Element ID + LDraw suffix + cross-system color IDs without requiring field-level schema agreement), combined with a thin applied API schema that contributes no typed properties beyond a discovery marker — making the existence of source identifiers machine-discoverable while deferring schema agreement on the content to domain-specific follow-ons. This parallels the `UsdMediaAssetPreviewsAPI` precedent: an applied schema that provides a described spec and convenience API over a dictionary, without defining the full field set as typed properties in `UsdPrimDefinition`.
