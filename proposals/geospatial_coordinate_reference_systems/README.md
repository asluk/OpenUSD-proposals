...
- [Runtime coordinate transformation](#runtime-coordinate-transformation)
  - [Runtime behavior](#runtime-behavior)
    - [Which CRS applies to a prim](#which-crs-applies-to-a-prim)
    - [Which CRS the scene resolves into](#which-crs-the-scene-resolves-into)
    - [What an anchor establishes](#what-an-anchor-establishes)
    - [Precision, axis order, units, and epoch](#precision-axis-order-units-and-epoch)
    - [When the transform cannot be computed](#when-the-transform-cannot-be-computed)
    - [Where the behavior can run](#where-the-behavior-can-run)
    - [How closely independent runtimes agree](#how-closely-independent-runtimes-agree)
    - [What a checker can catch at authoring time](#what-a-checker-can-catch-at-authoring-time)
    - [What this description does not settle](#what-this-description-does-not-settle)
    - [Implementations and measurements](#implementations-and-measurements)
  - [Default Implementation using the "PROJ" library](#default-implementation-using-the-proj-library)
...
3. **Third-party library abstraction.**
   ...
   The behavior such a plugin has to deliver is described in
   [Runtime behavior](#runtime-behavior);
